  ...
  return async function handler(req, res) {
    ...
    if (!rate.success) {
      const result = withMetadata({
        ...
        message: 'Interpretation capacity is temporarily limited. The original query will be used.',
        suggestions: [queryInfo.query], specificityLevel: 'unknown',
      }, { source: 'fallback', cacheStatus: cacheRead.status === 'miss' ? 'miss' : 'error', errorCode: 'RATE_LIMIT', timings });
      result.timings.totalMs = deadline.elapsedMs();
    ...
    try {
      const raw = await deadline.run('interpret-provider-call', () => providerLookup(queryInfo, {
        deadline,
