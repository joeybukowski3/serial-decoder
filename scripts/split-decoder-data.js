...
function buildBundle(source, category) {
  const dataStart = source.indexOf('var decoderData = {');
  if (dataStart === -1) throw new Error('Missing decoderData object in decoder-data.js');

  const prelude = source.slice(0, dataStart).trim();
  const categoryObject = extractCategoryObject(source, category.key);
