...
async function requireCount(locator, label) {
  if ((await locator.count()) < 1) throw new Error(`${label} is missing`);
}
...
try {
  ...
  page.on('console', message => {
    if (message.type() === 'error' && !/ERR_NAME_NOT_RESOLVED|ERR_BLOCKED_BY_CLIENT|doubleclick|googlesyndication|adtrafficquality/i.test(message.text())) consoleErrors.push(message.text());
  });
  page.on('pageerror', error => pageErrors.push(error.message));
  page.on('request', request => {
    if (/generativelanguage\.googleapis\.com|api\.groq\.com/i.test(request.url())) providerRequests.push(request.url());
  });
  ...
  await requireVisible(page.locator('#decodeBtn'), 'Decode button');
  await requireVisible(page.locator('#smartLookupBtn'), 'Smart Lookup button');
  await requireCount(page.locator('#replaceAction'), 'Replacement/LKQ action');
  await requireCount(page.locator('#replacementCategory'), 'Replacement category control');
  mark('homepage, Smart Lookup, and LKQ controls verified');

  await fillDecode(page, 'lg', '412TATG1H105', 'WM3470HWA');
  await page.click('#decodeBtn');
  await requireVisible(page.locator('#serialResults'), 'Serial results');
  await page.waitForFunction(() => document.querySelector('#resultYear')?.textContent?.trim() === '2004/2014/2024');
  await page.screenshot({ path: `${outputDir}/01-immediate-ambiguous-result.png`, fullPage: true });
  mark('immediate ambiguous result captured');

  await requireVisible(page.locator('.serial-refinement-status--checking'), 'Refinement loading status');
  await page.screenshot({ path: `${outputDir}/02-refinement-loading.png`, fullPage: true });
  mark('loading state captured');

  release();
  await requireVisible(page.locator('.serial-refinement-status--resolved'), 'Resolved refinement status');
  await page.waitForFunction(() => document.querySelector('#resultYear')?.textContent?.trim() === '2014');
  await requireVisible(page.locator('.serial-refinement-evidence summary'), 'Evidence used summary');
  await page.screenshot({ path: `${outputDir}/03-successful-mocked-resolution.png`, fullPage: true });
  mark('mocked resolution captured');

  await page.selectOption('#brand', 'kenmore');
  await page.waitForTimeout(100);
  await requireVisible(page.locator('#kenmoreModelField'), 'Kenmore model-prefix field');
  await page.fill('#kenmoreModelPrefix', '401');
  mark('Kenmore prefix field verified');
  ...
  await requireVisible(unavailable.locator('[data-serial-refinement-retry="1"]'), 'Retry button');
  await unavailable.waitForFunction(() => document.querySelector('#resultYear')?.textContent?.trim() === '1994/2024');
  await unavailable.screenshot({ path: `${outputDir}/04-unavailable-retry.png`, fullPage: true });
  mark('unavailable and Retry state captured');
  ...
  await requireVisible(electronics.locator('#serialResults'), 'Vizio results');
  await electronics.waitForFunction(() => /2007/.test(document.querySelector('#resultYear')?.textContent || ''));
  mark('Vizio model-primary behavior verified');
  ...
  await requireVisible(mobilePage.locator('.serial-refinement-status--resolved'), 'Mobile resolved status');
  await requireVisible(mobilePage.locator('#serialResults'), 'Mobile results');
  await mobilePage.screenshot({ path: `${outputDir}/05-mobile-result.png`, fullPage: true });
  mark('mobile result captured');
  ...
  await writeFile(`${outputDir}/smoke-results.txt`, `${milestones.join('\n')}\nNo live Gemini or Groq requests observed.\n`, 'utf8');
  await mobile.close();
  await desktop.close();
  console.log('Final-build smoke checks and five screenshot captures passed.');
} catch (error) {
  await writeFile(`${outputDir}/error.txt`, `${error.stack || error.message}\n\nCompleted milestones:\n${milestones.join('\n')}\n`, 'utf8');
  throw error;
} finally {
  await browser.close();
}
