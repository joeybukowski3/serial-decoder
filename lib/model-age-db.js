...
export function extractLocalModelAgeLookupTerms(query) {
  ...
  // Preserve a model value exactly as entered when the query uses an explicit
  // label. Generic tokenization below deliberately removes punctuation, which
  // is useful for matching but would otherwise turn "M321i-A2" into
  // "M321iA2" in the user-facing enteredModel diagnostic.
  const labeledModel = rawQuery.match(
    /\bmodel(?:\s+number)?\s*[:=]\s*([A-Za-z0-9][A-Za-z0-9 ._-]*?)(?=\s*(?:\||,|;|\b(?:serial|brand|category|type)\b\s*[:=])|$)/i
  );
  if (labeledModel?.[1]) terms.push(labeledModel[1].trim());
...
export function findExactLocalModelAgeMatch(records, query, brand) {
  ...
  // Verified exact evidence first, through the shared matcher that serial
  // refinement also uses. `buildSearchTerms` below covers `model` + `aliases`
  // only, so a verified `exactAlias` (e.g. GFW850SPN0DG for canonical
  // GFW850SPNDG) was previously invisible to Smart Lookup even though serial
  // refinement resolved it correctly. `exactAliases` is checked with strict
  // equality only and is deliberately NOT folded into buildSearchTerms, which
  // feeds Levenshtein/prefix fuzzy scoring.
  const exact = matchExactModelEvidence(records, query, { brand });
  // An ambiguous token must not silently pick a record; fall through to the
  // existing (brand-scoped) term matching, which cannot invent an exact
  // identity on its own.
  if (exact.record && !exact.ambiguous) {
    return {
      record: exact.record,
      matchType: exact.matchedBy === 'canonical-model' ? 'normalized-exact' : 'alias-exact',
      matchedBy: exact.matchedBy,
      canonicalModel: exact.canonicalModel,
      enteredModel: exact.enteredModel,
      verifiedExact: true,
    };
  }
...
function getExactManufactureYear(record) {
  if (record.individualManufactureYear != null) return record.individualManufactureYear;
...
export function formatLocalModelAgeMatch(record, options = {}) {
  ...
  return {
    ...
    // The entered value is preserved verbatim; the canonical model is exposed
    // alongside it so a verified exact-alias hit can show both without the
    // user's input appearing to have been silently corrected.
    model: options.enteredModel || record.displayModel || record.model || null,
    canonicalModel: record.model || null,
    enteredModel: options.enteredModel || null,
    matchedBy: options.matchedBy || null,
    verifiedExact: Boolean(options.verifiedExact),
    ...
    estimatedYearType: getExactManufactureYear(record) != null
      ? 'individual-manufacture'
      : (record.estimatedYearType || (record.introductionYear != null ? 'model-introduction' : null)),
    introductionYear: record.introductionYear ?? null,
    productionRange: record.productionRangeObject || null,
    individualManufactureYear: getExactManufactureYear(record),
    yearRange: record.productionRange || null,
    bestEstimateYear: record.modelYear ?? record.bestEstimateYear ?? null,
    estimatedRange: record.productionRangeObject || null,
    rangeLabel: record.rangeLabel || null,
    estimateBasis: record.estimateBasis || null,
    confidence: record.confidence || null,
    identityConfidence: record.identityConfidence || null,
    timingConfidence: record.timingConfidence || null,
    recognizedBrand: record.brand || null,
    recognizedCategory: record.category || null,
    recognizedModel: record.model || null,
    likelyProduct: record.likelyProduct || null,
    serialNeededForExactUnitDate: record.serialNeededForExactUnitDate === true,
    specificityLevel: record.model ? 'specific' : 'brand-only',
    inventionSummary: null,
    refinementSuggestion: getExactManufactureYear(record) == null
      ? 'Use a supported serial decode or dated manufacturing label to narrow the estimate for the individual unit.'
      : 'For the most accurate result, verify the exact brand and complete model number from the product label.',
