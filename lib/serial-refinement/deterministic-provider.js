...
import { buildSharedModelIdentity } from '../model-evidence/shared-model-identity.js';
...
export async function callDeterministicSerper(request, options = {}) {
  ...
  const modelIdentity = options.modelIdentity || buildSharedModelIdentity({
    brand: request.brand,
    model: request.model,
  ...
  const sharedEvidence = await (options.modelEvidenceLookup || lookupModelEvidence)({
    ...
    model: request.model,
    category: request.category,
    purpose: options.purpose || 'model_refinement',
    deadline,
    requestContext: {
      consumer: options.consumer || 'model_refinement',
      requestId: options.requestId || null,
      scoringPath: options.scoringPath || 'phase1-deterministic-evaluator',
    },
  }, {
    cache,
    redis: options.redis || null,
  ...
  return {
    ...
    candidateYears: request.candidateYears,
    localModelEvidence: options.localModelEvidence ?? null,
    modelIdentity: sharedEvidence.modelIdentity || modelIdentity,
    matchedIdentity: sharedEvidence.matchedIdentity || null,
    searchedModels: sharedEvidence.requestedIdentity?.searchedModels
      || modelIdentity.searchModels
      || [request.model],
    evidenceMatchModel: sharedEvidence.providerSummary?.evidenceMatchModel
      || sharedEvidence.matchedIdentity?.model
      || null,
    serper: null,
    gemini: {
    ...
    failureCategory: sharedEvidence.failureCategory || null,
    lifecycle: sharedEvidence.lifecycle || null,
