...
function isSafeAliasMatch(inputCompact, record) {
  ...
  // Permit a user to include the brand before the complete model value, but
  // never let a trimmed family prefix or omitted revision behave as exact.
  if (inputCompact.endsWith(exactModel) && inputCompact.length > exactModel.length) return true;

  return (record.exactAliases || []).some((alias) => compactModelValue(alias) === inputCompact);
}
...
export async function findLocalRefinementEvidence({ brand, model, dbPath } = {}) {
  ...
  if (match && compactModelValue(match.model) !== inputCompact) {
    const exactAlias = (match.exactAliases || []).find((alias) => compactModelValue(alias) === inputCompact);
    if (exactAlias) {
      matchedAlternative = {
        value: match.model,
        change: `${exactAlias}\u2192${match.model} (validated exact model alias)`,
        index: null,
        validated: true,
      };
    }
