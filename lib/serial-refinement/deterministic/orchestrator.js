...
export async function gatherEvidence(input, options = {}) {
  ...
  const baseline = await searchWithCache({ brand, model, category, allowBrandless: options.allowBrandlessSearch === true }, 'baseline', {
    ...options,
    searchMaxMs: Math.min(SERPER_TIMEOUT_MS, totalSearchBudgetMs),
  });
  ...
  if (!sufficient) {
    ...
    documentFocused = await searchWithCache(
      { brand, model, category: 'manual specifications', allowBrandless: options.allowBrandlessSearch === true },
      'document-focused',
