...
export const DATE_MEANINGS = new Set([
  'publication_date', 'listing_publication', 'product_launch', 'product_available',
  'production_start', 'production_end', 'discontinuation', 'manual_published',
...
export const MATCH_TYPES = new Set(['exact', 'variant', 'family', 'mismatch', 'unknown']);
export const DATE_PRECISIONS = new Set(['day', 'month', 'year', 'approximate', 'unknown']);
export const EVIDENCE_TARGETS = new Set(['model_lifecycle', 'specific_unit', 'source_only']);
export const EXTRACTION_CONFIDENCES = new Set(['high', 'medium', 'low']);
...
- exactModelMatch: your suggested classification only. Application code independently verifies model identity and may downgrade this value.
- suggestedMatchType: your suggested identity classification — exact | variant | family | mismatch | unknown. This is advisory only.
...
- approximateEndYear: an explicitly stated ending year when the text gives a lifecycle range; otherwise null. Never infer an end year from publication timing.
- dateMeaning: classify what the date (either the raw date field or any date found in the text) represents — exactly one of: publication_date, listing_publication, product_launch, product_available, production_start, production_end, discontinuation, manual_published, review_published, owner_purchase, ownership_age, troubleshooting_date, page_updated, unknown. Use production_start, production_end, or discontinuation only when the source text explicitly ties that year to that lifecycle event; never infer a production boundary from publication, listing, review, manual, troubleshooting, or page-update timing. A generic "page last updated" or "in stock" freshness stamp is page_updated, NOT product_launch.
- datePrecision: day | month | year | approximate | unknown.
- evidenceTarget: model_lifecycle | specific_unit | source_only.
- extractionConfidence: high | medium | low, reflecting how explicit the source text is.
...
export function normalizeExtractedEvidence(parsed, evidenceItemCount) {
  ...
  return items
    ...
    .map((item) => ({
      ...
      suggestedMatchType: MATCH_TYPES.has(item.suggestedMatchType)
        ? item.suggestedMatchType
        : (item.exactModelMatch === true ? 'exact' : 'unknown'),
      ...
      approximateEndYear: Number.isInteger(item.approximateEndYear) ? item.approximateEndYear : null,
      dateMeaning: DATE_MEANINGS.has(item.dateMeaning) ? item.dateMeaning : 'unknown',
      datePrecision: DATE_PRECISIONS.has(item.datePrecision) ? item.datePrecision : 'unknown',
      evidenceTarget: EVIDENCE_TARGETS.has(item.evidenceTarget) ? item.evidenceTarget : 'source_only',
      extractionConfidence: EXTRACTION_CONFIDENCES.has(item.extractionConfidence)
