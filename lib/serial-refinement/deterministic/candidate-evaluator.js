 ...
 *
 * (1) EXPLICIT EVIDENCE (unchanged from the prior revision) — every fact
 *     with exactModelMatch=true, a non-marketplace domain, and a resolvable
 *     effective year is classified by dateMeaning into:
 *       LOWER_BOUND  (product_launch, product_available, explicitlyNewProduct)
 ...
 *       UPPER_BOUND  (publication_date, manual_published, review_published,
 *                    troubleshooting_date, explicitlyDiscontinued) — model
 *                    existed BY this date. Nearest at-or-before candidate
 *                    gets a small bonus; every later candidate is penalized.
 ...
 *     that, together, place the model in a period. Eligible sources are the
 ...
 *     it, and requires ALL of: at least one eligible era anchor exists,
...
// "Explicit launch, availability, production-window, ownership-age,
// purchase-age, and certification evidence should remain stronger than a
// generic era anchor" (product spec) — these three roles are that stronger
// tier, unchanged from the prior revision.
const LOWER_BOUND_MEANINGS = new Set(['product_launch', 'product_available']);
const POINT_MEANINGS = new Set(['owner_purchase', 'ownership_age']);
const UPPER_BOUND_MEANINGS = new Set(['publication_date', 'manual_published', 'review_published', 'troubleshooting_date']);
...
// dateMeaning is checked before the explicit flags: a source with a
// specific, classifiable dateMeaning should not be overridden by a
// secondary explicitlyDiscontinued/explicitlyNewProduct flag pulling it
// toward the wrong role (e.g. a manufacturer page stating "manufactured
// Oct 2014 to Nov 2019" is a lower-bound product_launch fact even though
// Gemini may also flag explicitlyDiscontinued=true for the same source).
function classifyRole(fact) {
  ...
  if (fact.explicitlyNewProduct) return 'lowerBound';
  if (fact.explicitlyDiscontinued) return 'upperBound';
  return 'none';
}
...
const MEANINGS_WHERE_DATE_FIELD_IS_PRIMARY = new Set([
  ...
  'publication_date', 'manual_published', 'page_updated',
]);
...
export function evaluateCandidates({ candidateYears, evidenceFacts, localModelEvidence }) {
  ...
  const eligibleFacts = evidenceFacts
    ...
    .filter((fact) => fact.exactModelMatch && !isMarketplaceDomain(fact.domain) && fact.effectiveYear != null);
  ...
  if (explicitAloneResolves) {
    ...
    if (narrowed.length === 1 && top.score >= MIN_SCORE_TO_RESOLVE) {
