...
export const SHARED_EVIDENCE_SCHEMA_VERSION = '1';
...
export const SHARED_EVIDENCE_TTL_SECONDS = 60 * 60 * 24 * 7;
...
export function createDeterministicCache({ redis, deadline } = {}) {
  ...
  return {
    ...
    async setSharedEvidence(input, value) {
      ...
      if (['timeout', 'error', 'unavailable'].includes(value.status)) return;
      if (value.status === 'partial' && value.failureCategory !== 'EVIDENCE_CONFLICT') return;
      const ttl = ['no_exact_evidence', 'variant_only'].includes(value.status)
