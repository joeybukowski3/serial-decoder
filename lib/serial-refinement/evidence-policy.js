...
export function evaluateEvidencePolicy(evidenceRecords, options = {}) {
  ...
  if (official.length) {
    let range = intersectRanges(official.map((record) => record._range));

    // Official evidence can be partial (e.g. only productionEnd from a
    // current listing). Fill missing bounds from secondary evidence rather
    // than discarding it outright, while keeping official-derived bounds
    // authoritative wherever they exist.
    if (range.start == null || range.end == null) {
      const secondary = normalized.filter((record) =>
        SECONDARY_QUALITIES.has(record.quality) && record._range && hasUsableCitation(record));
      if (secondary.length) {
        const secondaryRange = intersectRanges(secondary.map((record) => record._range));
        if (range.start == null && secondaryRange.start != null) {
          range.start = secondaryRange.start;
        }
        if (range.end == null && secondaryRange.end != null) {
          range.end = secondaryRange.end;
        }
      }
    }
