...
function classifyQuality(item, source) {
  const declared = String(item.quality || '').toLowerCase();
  if (['official', 'strong-secondary', 'secondary', 'heuristic'].includes(declared)) return declared;
  const title = normalizeSourceTitle(source?.title);
  if (/\b(manufacturer|official|support|product|manual)\b/.test(String(item.type || '').toLowerCase())) return 'official';
  if (title) return 'strong-secondary';
  return 'heuristic';
}

function normalizeProviderEvidence(parsed, sources) {
  ...
  return items.map((item, index) => {
    ...
    return {
      ...
      quality: classifyQuality(item || {}, source),
...
export async function callGeminiGroundedSearch(request, options = {}) {
  ...
  const evidence = normalizeProviderEvidence(parsed, sources);
