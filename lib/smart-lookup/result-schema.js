...
const QUERY_SPECIFICITY_LEVELS = new Set([
  'exact-model', 'model-line', 'product-family', 'brand-category', 'brand-only', 'category-only', 'free-description', 'unusable',
]);
const CONTEXT_LEVELS = new Set(['exact-model', 'model-line', 'product-family', 'brand-category', 'category-history']);
...
const ESTIMATE_BASIS_LEVELS = new Set([
  'serial-decode', 'manufacturing-label', 'verified-exact-model',
  'verified-model-generation', 'verified-lineup-generation',
  'exact-model-dated-source', 'model-introduction', 'production-range',
  'model-line-generation', 'product-family-introduction', 'category-era', 'heuristic',
]);
...
const SOURCES = new Set(['local-db', 'decoder-verified', 'cache', 'static', 'serper', 'gemini', 'groq', 'xai', 'fallback', 'none']);
// 'openai-web' and 'xai-web' are real web-search-cited provider results;
// ungrounded variants mean the provider output had no server-controlled
// citations and must be shown as an estimate.
const EVIDENCE_SOURCES = new Set(['local-db', 'user-verified', 'serper-extracted', 'gemini-grounded', 'gemini-ungrounded', 'openai-web', 'openai-ungrounded', 'xai-web', 'xai-ungrounded', 'groq-ungrounded', 'heuristic', 'none']);
// Evidence sources permitted to carry citation links. Anything else has its
// sources array emptied, so a closed-book answer can never show links.
const CITED_EVIDENCE_SOURCES = new Set(['serper-extracted', 'gemini-grounded', 'openai-web', 'xai-web']);
...
// Separate from the pre-existing CONFIDENCE_LEVELS (which uses 'unknown' and
// is relied on by confidenceLevel): the usefulness-first identity/timing
// confidences use 'uncertain', and both spellings are accepted here so a
// provider using either vocabulary is normalized rather than dropped.
const IDENTITY_CONFIDENCE_LEVELS = new Set(['high', 'medium', 'low', 'uncertain', 'unknown']);

function normalizeStringList(value, max = 4, maxLength = 300) {
  if (!Array.isArray(value)) return [];
  return value.map((item) => cleanText(item, maxLength)).filter(Boolean).slice(0, max);
}

/**
 * Candidate products shown when identity is genuinely ambiguous. Deliberately
 * carries no URLs -- citations only ever come from real search annotations.
 */
function normalizeAlternativeMatches(value, max = 3) {
  if (!Array.isArray(value)) return [];
  return value
    .map((entry) => {
      if (!entry || typeof entry !== 'object') return null;
      const product = cleanText(entry.product || entry.name || entry.model, 160);
      if (!product) return null;
      return {
        product,
        reason: cleanText(entry.reason, 300) || null,
        confidence: normalizeSource(entry.confidence, IDENTITY_CONFIDENCE_LEVELS, 'low'),
      };
    })
    .filter(Boolean)
    .slice(0, max);
...
function normalizeManufactureYearCandidates(value, currentYear, trustedSerialEvidence) {
  if (!trustedSerialEvidence || value == null) return [];
  if (!Array.isArray(value)) throw new SmartLookupValidationError('INVALID_YEAR_CONTEXT');
  return [...new Set(value.slice(0, 4).map((entry) => {
    const year = Number(entry);
    if (!Number.isInteger(year) || year < 1800 || year > currentYear + 1) {
      throw new SmartLookupValidationError('INVALID_YEAR', 'manufactureYearCandidates contains an unsupported year.');
    }
    return year;
  }))].sort((a, b) => a - b);
}

function serialDetectedFor(queryInfo) {
  if (queryInfo?.serialSource !== 'labeled') return null;
  const token = cleanText(queryInfo.serialToken || queryInfo.serialIdentity, 80).toUpperCase();
  if (!token || !/^[A-Z0-9./-]+$/.test(token)) return null;
  return { token, action: 'use-decoder' };
}
...
export function normalizeSmartAgeResult(raw, options = {}) {
  ...
  const categoryEntryYear = normalizeYear(raw.categoryEntryYear, currentYear, 'categoryEntryYear');
  const familyIntroductionYear = normalizeYear(raw.familyIntroductionYear, currentYear, 'familyIntroductionYear');
  const lineIntroductionYear = normalizeYear(raw.lineIntroductionYear, currentYear, 'lineIntroductionYear');
  const generationRange = cleanText(raw.generationRange, 160) || null;
  ...
  // A vague brand/partial query must not carry a precise model date -- that
  // guard stays. But when research positively IDENTIFIED a specific commercial
  // product, the timing describes that identified product, not an arbitrary
  // guess from the brand, so suppressing it would discard the whole point of
  // the research ("Nintendo Switch 2" is brand-only to the local parser, yet
  // its release year is a well-established fact about a named product).
  // Deliberately requires BOTH a concrete product name and a non-weak identity
  // confidence, and never rescues an individual-unit manufacture year.
  const hasIdentifiedProduct = Boolean(cleanText(raw.likelyProduct, 160))
    && ['high', 'medium'].includes(String(raw.identityConfidence || '').toLowerCase());
  if (requestedSpecificity === 'generic' || requestedSpecificity === 'brand-only' || requestedSpecificity === 'partial') {
    if (!hasIdentifiedProduct) {
  ...
  const trustedSerialEvidence = options.allowSerialEvidence === true
    || source === 'local-db'
    || source === 'decoder-verified';
  const manufactureYearCandidates = normalizeManufactureYearCandidates(
    raw.manufactureYearCandidates,
    currentYear,
    trustedSerialEvidence
  );
  const manufactureDateAmbiguous = trustedSerialEvidence
    && raw.manufactureDateAmbiguous === true
    && manufactureYearCandidates.length > 0;
  let evidenceSource = normalizeSource(options.evidenceSource || raw.evidenceSource, EVIDENCE_SOURCES, 'none');
  // On a cache read there are no options.groundedSources, so citations must be
  // restored from the stored payload -- for EVERY cited evidence source, not
  // just Gemini. Restricting this to 'gemini-grounded' silently dropped the
  // citations from every cached OpenAI web result while leaving its
  // web-researched label intact, which is precisely the "claims research it
  // cannot show" state the source rules exist to prevent.
  const groundedSourcesInput = options.groundedSources !== undefined
    ? options.groundedSources
    : (CITED_EVIDENCE_SOURCES.has(options.evidenceSource || raw.evidenceSource) ? raw.sources : null);
  const sources = normalizeGroundedSources(groundedSourcesInput);
  // A cited evidence source requires at least one retrieved source; a call
  // that produced no citations is downgraded to the honest ungrounded label.
  if (evidenceSource === 'gemini-grounded' && !sources.length) evidenceSource = 'gemini-ungrounded';
  if (evidenceSource === 'openai-web' && !sources.length) evidenceSource = 'openai-ungrounded';
  if (evidenceSource === 'xai-web' && !sources.length) evidenceSource = 'xai-ungrounded';
  const retrievedAt = evidenceSource === 'gemini-grounded'
    || evidenceSource === 'openai-web'
  ...
  const estimatedYearType = individualManufactureYear != null
    ...
    : (introductionYear != null
      ? 'model-introduction'
      : (raw.estimatedYearType === 'model-production' && productionRange ? 'model-production' : null));
  ...
  const trustedFamilyContext = raw.yearContext?.source === 'local-seed'
    ...
    || (trustedSerialEvidence && raw.yearContext?.source === 'serial');
  let rawYearContext = requestedSpecificity === 'partial' && !trustedFamilyContext ? null : raw.yearContext;
  if (!trustedSerialEvidence
    && (rawYearContext?.type === 'manufacture-year' || rawYearContext?.type === 'manufacture-date')) {
    rawYearContext = null;
  }
  ...
  } else if (lineIntroductionYear != null) {
    derivedYearContext = { value: lineIntroductionYear, type: 'market-introduction', label: 'Product-line introduction', confidence: 'medium', isExactUnitDate: false };
  } else if (familyIntroductionYear != null) {
    derivedYearContext = { value: familyIntroductionYear, type: 'market-introduction', label: 'Product-family introduction', confidence: 'medium', isExactUnitDate: false };
  } else if (categoryEntryYear != null) {
    derivedYearContext = { value: categoryEntryYear, type: 'market-introduction', label: 'Brand/category history', confidence: 'medium', isExactUnitDate: false };
  ...
  const estimatedRange = individualManufactureYear != null
    ? { start: individualManufactureYear, end: individualManufactureYear, current: false, basis: 'serial-decode' }
    : (parseOpenRange(raw.estimatedRange, currentYear, 'estimatedRange')
      || (productionRange ? { ...productionRange, current: false } : null)
      || (introductionYear != null
        ? { start: introductionYear, end: null, current: true, basis: 'model-introduction' }
        : null));
  const derivedBestEstimateYear = individualManufactureYear
    ?? introductionYear
    ?? lineIntroductionYear
    ?? familyIntroductionYear
    ?? (productionRange ? Math.ceil((productionRange.start + productionRange.end) / 2) : null)
    ?? estimatedRange?.start
    ?? null;
  const bestEstimateYear = individualManufactureYear
    ?? normalizeYear(raw.bestEstimateYear, currentYear, 'bestEstimateYear')
    ?? derivedBestEstimateYear;
  const rangeLabel = (individualManufactureYear != null ? String(individualManufactureYear) : cleanText(raw.rangeLabel, 120))
    || (estimatedRange
      ? (estimatedRange.end == null
        ...
        : (estimatedRange.start === estimatedRange.end
          ...
          : `${estimatedRange.start}-${estimatedRange.end}`))
      : null);
  const derivedEstimateBasis = individualManufactureYear != null
    ? 'serial-decode'
    : (source === 'local-db' || source === 'decoder-verified'
      ? 'verified-exact-model'
      : (productionRange
        ? 'production-range'
        : (lineIntroductionYear != null
          ? 'model-line-generation'
          : (familyIntroductionYear != null
            ? 'product-family-introduction'
            : (introductionYear != null
              ? 'model-introduction'
              : (categoryEntryYear != null || raw.contextLevel === 'category-history' ? 'category-era' : 'heuristic'))))));
  const estimateBasis = individualManufactureYear != null
    ? 'serial-decode'
    : normalizeSource(raw.estimateBasis, ESTIMATE_BASIS_LEVELS, derivedEstimateBasis);
  const confidence = normalizeSource(raw.confidence || raw.confidenceLevel || raw.timingConfidence,
  ...
  return {
    ...
    estimateBasis,
    confidence,
    summary: cleanText(raw.summary, 1000) || null,
    timeline: cleanText(raw.timeline, 1000) || null,
    inventionSummary: cleanText(raw.inventionSummary, 1000) || null,
    contextLevel: normalizeSource(raw.contextLevel, CONTEXT_LEVELS, null),
    historicalContext: cleanText(raw.historicalContext, 1200) || null,
    categoryEntryYear,
    familyIntroductionYear,
    lineIntroductionYear,
    generationRange,
    contextConfidence: normalizeSource(raw.contextConfidence, IDENTITY_CONFIDENCE_LEVELS, null),
    refinementSuggestion: cleanText(raw.refinementSuggestion, 500) || null,
    notes: cleanText(raw.notes, 2000) || null,
    // Usefulness-first identification fields. These let a researched result
    // state WHAT the product is and how confident it is, separately from
    // whether an individual unit's manufacture date could be established --
    // the distinction the previous schema had no way to express.
    likelyProduct: cleanText(raw.likelyProduct, 160) || null,
    estimatedEra: cleanText(raw.estimatedEra, 120) || null,
    releaseDate: cleanText(raw.releaseDate, 40) || null,
    identityConfidence: normalizeSource(raw.identityConfidence, IDENTITY_CONFIDENCE_LEVELS, null),
    timingConfidence: normalizeSource(raw.timingConfidence, IDENTITY_CONFIDENCE_LEVELS, null),
    caveats: normalizeStringList(raw.caveats),
    assumptions: normalizeStringList(raw.assumptions),
    alternativeMatches: normalizeAlternativeMatches(raw.alternativeMatches),
    serialNeededForExactUnitDate: raw.serialNeededForExactUnitDate === true,
    // Server-derived from provider metadata when available; the raw provider
    // JSON is only a fallback and can never upgrade a non-search result.
    webSearchUsed: (options.webSearchUsed ?? raw.webSearchUsed) === true,
    evidence: normalizeEvidence(raw.evidence),
    sources: CITED_EVIDENCE_SOURCES.has(evidenceSource) ? sources : [],
    retrievedAt,
    groundedFallback,
    serialDetected: serialDetectedFor(queryInfo),
    serialLocation: trustedSerialEvidence ? (cleanText(raw.serialLocation, 500) || null) : null,
    serialRule: trustedSerialEvidence ? (cleanText(raw.serialRule, 500) || null) : null,
    ...
    series: cleanText(raw.series || raw.seriesLine || queryInfo.seriesLine, 120) || null,
