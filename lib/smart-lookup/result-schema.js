...
export function normalizeSmartAgeResult(raw, options = {}) {
  ...
  const estimatedRange = parseOpenRange(raw.estimatedRange, currentYear, 'estimatedRange')
  ...
  const derivedBestEstimateYear = individualManufactureYear
    ...
    ?? null;
  const bestEstimateYear = normalizeYear(raw.bestEstimateYear, currentYear, 'bestEstimateYear')
    ?? derivedBestEstimateYear;
  const rangeLabel = cleanText(raw.rangeLabel, 120)
    || (estimatedRange
  ...
  const estimateBasis = normalizeSource(raw.estimateBasis, ESTIMATE_BASIS_LEVELS, derivedEstimateBasis);
