...
export const GENERAL_FAMILY_SEEDS = [
  ...
  // Lenovo ThinkSystem ST50 tower-server generations. Deliberately scoped to
  // this product line only -- other Lenovo server families are not assumed to
  // share the same introduction timing. V2/V3 are explicit later generations
  // and must never inherit the original ST50 production window. Official
  // evidence: Lenovo ST50 Setup Guide (machine types 7Y48/7Y49, © 2018) and
  // Lenovo Press ST50 product guide first published 5 November 2018.
  {
    brand: 'Lenovo',
    category: 'tower server',
    productRole: 'tower server',
    familyId: 'thinksystem-st50',
    familyName: 'ThinkSystem ST50',
    familyPattern: /\bTHINKSYSTEM\s*ST\s*50\b|\bST\s*50\b|\b7Y4[89]\b/i,
    modelLineId: null,
    modelLineName: null,
    // Group 1 = V2/V3 generation; group 2 = original-generation machine type.
    modelLinePattern: /\b(?:THINKSYSTEM\s*)?ST\s*50(?:\s*V\s*([23]))?\b|\b(7Y4[89])\b/i,
    dynamicModelLine: true,
    buildModelLineId: (match) => {
      if (match[2]) return 'thinksystem-st50';
      if (match[1] === '2') return 'thinksystem-st50-v2';
      if (match[1] === '3') return 'thinksystem-st50-v3';
      return 'thinksystem-st50';
    },
    buildModelLineName: (match) => {
      if (match[2]) return `ThinkSystem ST50 (${String(match[2]).toUpperCase()})`;
      if (match[1] === '2') return 'ThinkSystem ST50 V2';
      if (match[1] === '3') return 'ThinkSystem ST50 V3';
      return 'ThinkSystem ST50';
    },
...
export const GENERAL_FAMILY_CONTEXT_BUSINESS_COMPUTERS = {
  ...
  // Official Lenovo documentation only. Original ST50: Setup Guide machine
  // types 7Y48/7Y49 (© 2018) and Lenovo Press product guide first published
  // 5 November 2018; later generations are explicitly named ST50 V2 / ST50 V3
  // and do not inherit the original production window. No individual unit
  // manufacture year is inferred from model-line identity alone.
  'Lenovo:thinksystem-st50': {
    displayName: 'Lenovo ThinkSystem ST50 tower server',
    confidence: 'medium',
    familyRange: { start: 2018, end: 2023, current: false, basis: 'model-line-history' },
    modelLineRanges: {
      // Original generation only. Conservative supported window: documented
      // 2018 introduction through the early 2020s (not an individual manufacture year).
      'thinksystem-st50': { start: 2018, end: 2023, current: false, basis: 'model-line-history' },
      // Explicit later generations: recognized for identity, but no invention of
      // an introduction year without checked-in official evidence for those lines.
      'thinksystem-st50-v2': { start: null, end: null, current: true, basis: 'model-line-history' },
      'thinksystem-st50-v3': { start: null, end: null, current: true, basis: 'model-line-history' },
    },
    generationSummary: [
      'The original ThinkSystem ST50 tower server is documented by Lenovo with machine types 7Y48 and 7Y49; Lenovo materials carry 2018 copyright and the Lenovo Press ST50 product guide was first published in November 2018.',
      'Later products are explicitly named ThinkSystem ST50 V2 and ThinkSystem ST50 V3, so a label that says only ST50 most likely refers to the original generation rather than V2 or V3.',
      'An original-generation production period of approximately 2018 through the early 2020s is a model-line estimate, not the manufacture date of an individual unit.',
    ],
    configurationVariants: [
      'Original ThinkSystem ST50 (machine types 7Y48, 7Y49)',
      'ThinkSystem ST50 V2 (later generation; named explicitly on the product)',
      'ThinkSystem ST50 V3 (later generation; named explicitly on the product)',
    ],
    refinementIdentifiers: [
      'Enter the machine type from the system label, such as 7Y48 or 7Y49 for the original ST50 generation.',
      'Enter a V2 or V3 designation when the product label names ThinkSystem ST50 V2 or ST50 V3.',
      'Enter the full Lenovo model or serial/MTM label for unit-specific lookup through Lenovo support; Smart Lookup does not decode Lenovo serials locally.',
    ],
    serviceTagNote: 'Lenovo machine type and serial/MTM numbers on the case label identify one exact original configuration through Lenovo\'s own lookup tool -- Smart Lookup does not decode them locally.',
  },
};
