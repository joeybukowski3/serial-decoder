...
export function callSmartLookupXaiAgeProvider(queryInfo, options = {}) {
  return callXaiResponses(options.prompt || buildOpenAiAgeProviderPrompt(queryInfo), {
    ...options,
    fallbackUsed: false,
    primaryProvider: 'xai',
    xaiStage: options.xaiStage || 'age-provider-xai',
  });
}
...
function errorForStatus(status, bodyText, options = {}) {
  ...
  if (status === 429) {
    if (/insufficient_quota|billing_hard_limit_reached|credits?\s+exhausted|quota\s+exhausted|account\s+quota|billing/i.test(code)) {
      return new SmartLookupProviderError('XAI_QUOTA_EXHAUSTED', 'xAI account quota or billing limit reached', common);
    }
