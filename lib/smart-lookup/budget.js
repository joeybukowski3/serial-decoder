...
export async function recordProviderAttemptMetrics(redis, kind, actualAttempts, deadline, options = {}) {
  ...
  try {
    ...
    return {
      status: 'recorded',
      // Per-request count. The Redis INCRBY return values below are daily
      // cumulative counters and must never be logged under this field name.
      actualProviderAttemptCount: attempts,
      cumulativeProviderAttemptCount: Number(serviceCount || 0),
      combinedCumulativeProviderAttemptCount: Number(combinedCount || 0),
    };
  } catch (_) {
...
export function providerAttemptCountFromMetadata(metadata, errorCode = null) {
  if (Number.isInteger(metadata?.actualProviderAttemptCount) && metadata.actualProviderAttemptCount >= 0) {
    return metadata.actualProviderAttemptCount;
  }
