...
/**
 * One OpenAI Responses call for Smart Lookup age research.
 *
 ...
 *
 * Ordinary requests disable the optional xAI failover. It remains available
 * only for a future explicitly controlled deep-research or immediate-failure
 * route; it is never enabled by default and cannot silently double-charge.
 */
...
function errorForStatus(status, bodyText) {
  ...
  if (status === 429) {
    if (/insufficient_quota|billing_hard_limit_reached|credits?\s+exhausted|quota\s+exhausted|account\s+quota|billing/i.test(code)) {
      return new SmartLookupProviderError('OPENAI_QUOTA_EXHAUSTED', 'OpenAI account quota or billing limit reached', {
        status, provider: 'openai',
      });
    }
