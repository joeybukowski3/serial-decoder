...
function buildGeneralFamilyResult(queryInfo, tier) {
  ...
  // Prefer an explicit model-line entry when present -- including an intentional
  // "recognized but no dated window" entry (start: null) so a later generation
  // never inherits an older sibling's production range via familyRange fallback.
  const hasExplicitModelLineRange = Boolean(
    tier === 'model-line'
    && context?.modelLineRanges
    && Object.prototype.hasOwnProperty.call(context.modelLineRanges, queryInfo.modelLineId)
  );
  const modelLineRange = tier === 'model-line'
    ? (hasExplicitModelLineRange
      ? context.modelLineRanges[queryInfo.modelLineId]
      : familyRange)
    : null;
  ...
  const estimateBasis = tier === 'model-line'
    ? 'model-line-generation'
    : 'product-family-introduction';
  const hasDatedWindow = Boolean(activeRange && activeRange.start != null);
  // An explicit later generation with no dated window must not inherit the
  // parent family's introduction year into bestEstimateYear / familyIntroductionYear.
  const undatedExplicitModelLine = Boolean(
    tier === 'model-line' && hasExplicitModelLineRange && !hasDatedWindow
  );
  const productionRange = hasDatedWindow && activeRange.end != null
    ? { start: activeRange.start, end: activeRange.end, basis: activeRange.basis || estimateBasis }
    : null;
  const estimatedRange = hasDatedWindow
    ? {
        start: activeRange.start,
        end: activeRange.end == null ? null : activeRange.end,
        current: Boolean(activeRange.current),
        basis: activeRange.basis || estimateBasis,
      }
    : null;
  ...
  return {
    ...
    outcome: yearContext.type === 'unknown'
      ? (tier === 'model-line' ? 'model-line-recognized' : 'product-recognized-year-unverified')
      : (tier === 'model-line' ? 'model-line-year-context' : 'product-family-year-context'),
    resultType: tier === 'model-line' ? 'model-line-recognized' : 'product-family-recognized',
    brand,
    displayName,
    contextLevel,
    historicalContext: notesParts.join(' '),
    familyIntroductionYear: undatedExplicitModelLine ? null : (familyRange?.start || null),
    lineIntroductionYear: (modelLineRange && modelLineRange.start != null) ? modelLineRange.start : null,
    generationRange: hasDatedWindow
      ? (activeRange.end != null ? `${activeRange.start}-${activeRange.end}` : `${activeRange.start}-present`)
      : null,
    ...
    estimatedRange,
    estimateBasis,
    bestEstimateYear: hasDatedWindow ? activeRange.start : null,
    individualManufactureYear: null,
    // Lets normalizeSmartAgeResult keep production windows for partial-tier
    // model-line/family cards (it only preserves precise timing when a
    // commercial product was positively identified at medium+ confidence).
    likelyProduct: displayName,
    identityConfidence: context?.confidence || 'medium',
...
export function buildDeterministicBroadResult(queryInfo) {
  ...
  if (queryInfo.specificityLevel === 'brand-only') {
    ...
    const serviceTagOnly = Boolean(queryInfo.serviceTagIdentity && !queryInfo.modelIdentity);
    const recommendedIdentifiers = serviceTagOnly
      ? [
          'Use the manufacturer support lookup for the service or asset tag.',
          'Enter the commercial product name or complete model number for Smart Lookup research.',
        ]
      : [
          'Enter the complete model number from the product label.',
    ...
    return {
      ...
      refinementSuggestion: recommendedIdentifiers[0],
      notes: serviceTagOnly
        ? 'A service or asset tag identifies one unit through the manufacturer, but it is not a model number and Smart Lookup does not decode it as product-age evidence.'
        : `${queryInfo.brand || 'This brand'} produces multiple ${category} product generations, so a brand-only search cannot establish a model introduction year or an individual manufacture date.`,
      ...
      refinementNeeded: true,
      refinementReason: serviceTagOnly
        ? 'The query supplied a unit service tag but no product model or category.'
        : queryInfo.querySpecificity === 'brand-only'
