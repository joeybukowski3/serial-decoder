...
function buildProductFamilyResult(queryInfo, specificityLevel) {
  ...
  if (exactModel) {
    ...
    notesParts.push(`This looks like an LG ${queryInfo.productFamily} OLED TV product-family search, but it is not the exact model number.`);
    if (context?.familyContext) notesParts.push(context.familyContext);
  } else {
    notesParts.push(`This looks like a ${brand} ${sizeText}${queryInfo.productFamily} ${category} description, but it is not the exact model number.`);
    if (context?.variantsNote) notesParts.push(context.variantsNote);
  ...
  return {
    ...
    refinementSuggestion: exactModel
      ...
      : context && context.exampleModels.length && brand === 'LG'
        ? `Look for an exact model number such as ${context.exampleModels.join(', ')} on the rear label, box, receipt, or TV settings.`
