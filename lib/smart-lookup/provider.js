...
function familySpecificityBlock(queryInfo) {
  ...
  if (tier === 'brand-category') {
    ...
    return `\n\nThis query identifies only a BRAND and CATEGORY (${label}), not any specific model, model line, or product family -- no model number was provided or could be inferred. Research only what is defensible at this broad level: known product eras for this brand/category, broad introduction or availability periods, common model-number formats this brand uses for this category, and what cannot be determined without the model number. Do not select one arbitrary model and report its date. Do not claim one manufacture year. Do not invent a specific product family or model line that was not part of the query. Do not present a typical service-life span as if it were this item's age -- service life and product age are different things. Tell the user which identifier (model number, serial number, or nameplate/rating-plate label) would let Smart Lookup narrow this to an exact answer.
...
Return model-level information only. Do not estimate the manufacture date of an individual physical unit from a model number. Do not claim that you performed live web research. If the model token is partial, do not silently complete it; return suggestions instead. If the query is generic or brand-only, do not return a precise model date.${familySpecificityBlock(queryInfo)}
...
Return model-level information only. Do not estimate the manufacture date of an individual physical unit from a model number. If the model token is partial, do not silently complete it; return suggestions instead. If the query is generic or brand-only, do not return a precise model date.${familySpecificityBlock(queryInfo)}
...
// about the *replacement* claim, not the *age* claim.
function lkqOverclaimGuard(queryInfo) {
  const tier = queryInfo.querySpecificity;
  if (tier === 'exact-model' || !tier) return '';
  if (tier === 'model-line' || tier === 'product-family') {
    return `\n\nThis query identifies a ${tier === 'model-line' ? 'model line' : 'product family'}, not one exact original unit. Only propose successorStatus/replacement information at the same broad level (the current generation of this same line/family), never one arbitrary current product presented as THE successor to the entire line/family. If you cannot name a genuinely singular current continuation, use successorStatus.type "none" / replacementRelationship "none-found" rather than guessing.`;
  }
  return `\n\nThis query identifies only a brand and/or category, not a specific product line. Do not name any single current product as a direct or same-series successor -- there is no specific original item to succeed. Use successorStatus.type "none" and replacementRelationship "none-found" unless the query itself already narrows to one clear line.`;
}
