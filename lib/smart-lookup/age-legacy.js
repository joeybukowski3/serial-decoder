...
import { resolveVizioModelGeneration } from '../vizio/model-generation-resolver.js';
...
const HVAC_CONFIG = [
  ...
  { brand: 'Lennox', aliases: ['lennox'], type: 'lennoxPPYYM' },
  { brand: 'York', aliases: ['york'], type: 'yorkSerial' },
];
...
// York modern/legacy months: A-H and K-N (I and J excluded).
const YORK_MONTHS = {
  A: 'January', B: 'February', C: 'March', D: 'April',
  E: 'May', F: 'June', G: 'July', H: 'August',
...
function shouldReturnCandidate(candidate, term, inferredBrand) {
  ...
  // Verified model-generation dates are identity-specific. A close/prefix
  // match (for example M321i-Z9 near M321i-A2) must not inherit the exact
  // record's year range; unsupported suffixes continue through the broader
  // Smart Lookup path instead of receiving an invented deterministic year.
  if (candidate.record?.estimateBasis === 'verified-model-generation') return false;
...
export async function findLocalModelAgeResult(query, normalizedQuery = String(query || '').toLowerCase()) {
  ...
  const enteredBrandMatch = String(query || '').match(
  ...
  // VIZIO generation evidence is maintained separately from the generic
  // model-age database because it supports exact records plus constrained
  // official-lineup patterns. Respect an entered non-VIZIO brand here so the
  // route can preserve and disclose the conflict through its brand-agnostic
  // conflict probe below.
  if ((!enteredBrand || brandKey(enteredBrand) === 'vizio')
    && (!inferredBrand || brandKey(inferredBrand) === 'vizio')) {
    for (const term of terms) {
      const vizio = await resolveVizioModelGeneration(term);
      if (vizio) return applyEraHints(vizio, normalizedQuery);
    }
  }
  ...
  for (const term of extractLocalModelAgeLookupTerms(query)) {
    ...
    if (vizio) {
      return {
        record: {
          brand: vizio.brand,
          model: vizio.canonicalModel,
          category: vizio.category,
          series: vizio.series,
        },
        matchedBy: vizio.matchedBy,
        enteredModel: term,
        localResult: applyEraHints(vizio, String(query || '').toLowerCase()),
      };
    }
    ...
    if (match.record) {
      return {
        ...
        enteredModel: term,
        localResult: applyEraHints(formatLocalModelAgeMatch(match.record, {
          confidence: 1,
          matchType: match.matchedBy === 'canonical-model' ? 'normalized-exact' : 'alias-exact',
          matchedBy: match.matchedBy,
          verifiedExact: true,
          enteredModel: term,
...
export function decodeHvacSerial(query, normalizedQuery = String(query || '').toLowerCase(), queryInfo = null, options = {}) {
  ...
  function exactSerialResult(year, month, detail, serialRule) {
    return applyEraHints({
      brand: config.brand,
      estimatedYear: year,
      estimatedYearType: 'individual-manufacture',
      individualManufactureYear: year,
      manufactureDateAmbiguous: false,
      manufactureYearCandidates: [year],
      yearContext: {
  ...
  function multiYearSerialResult(years, month, detail, serialRule) {
    const manufactureYearCandidates = [...years]
      .map((year) => Number(year))
      .filter((year) => Number.isFinite(year) && year <= currentYear + 1)
      .sort((a, b) => a - b);
    if (!manufactureYearCandidates.length) return null;
    return applyEraHints({
      brand: config.brand,
      ...
      notes: `${detail}${month ? ` Month ${month}.` : ''} The serial year letter maps to multiple supported manufacture years, so the serial alone does not establish one exact year.`,
      serialRule,
      yearRange: null,
      refinementNeeded: true,
      refinementReason: 'The York legacy year letter repeats on a multi-year cycle and does not establish one manufacture year without model-era evidence.',
      refinementSuggestion: 'Enter the complete model number or provide nameplate/model-era evidence to resolve the candidate year.',
      recommendedIdentifiers: [
        'Enter the complete model number from the same equipment nameplate.',
        'Provide an installation record or dated model-era document that can distinguish the serial cycle.',
  ...
  if (config.type === 'lennoxPPYYM') {
    const normalized = String(serial).replace(/\s+/g, '').toUpperCase();
    if (normalized.length < 5 || !/^\d{4}[A-HJ-M]/.test(normalized)) return null;
    const yy = normalized.substring(2, 4);
    const monthCode = normalized.charAt(4);
    const month = LENNOX_MONTHS[monthCode];
    if (!month) return null;
    const yyNum = Number(yy);
    if (!Number.isFinite(yyNum)) return null;
    const year = (yyNum >= 73 ? 1900 : 2000) + yyNum;
    if (year > currentYear + 1) return null;
    return exactSerialResult(
      year,
      month,
      `${month} ${year} matches the Lennox modern PPYYM serial pattern (plant ${normalized.substring(0, 2)}, year ${yy}, month ${monthCode}).`,
      'Lennox: positions 1-2 plant, 3-4 year, 5 month letter A-H/J-M (modern PPYYM).'
    );
  }

  if (config.type === 'yorkSerial') {
    let normalized = String(serial).replace(/\s+/g, '').toUpperCase();
    normalized = normalized.replace(/^\([A-Z]\)/, '');
    const modern = normalized.match(/^([A-Z])(\d)([A-HK-N])(\d)/);
    if (modern) {
      const year = 2000 + Number(modern[2] + modern[4]);
      if (year < 2004 || year > currentYear + 1) return null;
      const month = YORK_MONTHS[modern[3]];
      if (!month) return null;
      return exactSerialResult(
        year,
        month,
        `${month} ${year} matches the York modern L-D-L-D serial pattern (plant ${modern[1]}, year ${modern[2]}${modern[4]}, month ${modern[3]}).`,
        'York: modern plant letter + year digit + month letter + year digit (Oct 2004+).'
      );
    }
    const legacy = normalized.match(/^([A-Z])([A-HK-N])([A-Z])([A-Z])\d{6}$/);
    if (!legacy) return null;
    const month = YORK_MONTHS[legacy[2]];
    if (!month) return null;
    const years = YORK_LEGACY_YEARS[legacy[3]];
    ...
    if (years.length === 1) {
      ...
      return exactSerialResult(
        year,
        month,
        `${month} ${year} matches the York legacy letter-year serial pattern (plant ${legacy[1]}, month ${legacy[2]}, year letter ${legacy[3]}).`,
        'York: legacy four-letter prefix + six digits (1971-Oct 2004).'
      );
    }
    return multiYearSerialResult(
      years,
      month,
      `Month ${legacy[2]} and year letter ${legacy[3]} match the York legacy serial pattern (candidates ${years.join('/')}).`,
      'York: legacy four-letter prefix + six digits with multi-year letter cycle (1971-Oct 2004).'
    );
  }
  ...
  // Remaining wwYY brands (Carrier family / Trane) — not Lennox or York.
  const week = Number(match[1]);
  if (week < 1 || week > 53) return null;
  return ambiguousResult(
    match[2],
