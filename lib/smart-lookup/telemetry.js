...
export function logSmartLookup(logger, fields = {}) {
  const safe = {
    ...
    // Shared failure taxonomy (Phase 2) — optional, never raw user text.
    failureCategory: fields.failureCategory || null,
    failureStage: fields.failureStage || fields.timeoutStage || null,
    failureCode: fields.failureCode || fields.errorCode || null,
    resultTierReturned: fields.resultTierReturned || fields.resultEvidenceType || null,
    ...
    searchQueryCount: Number.isFinite(fields.searchQueryCount) ? fields.searchQueryCount : null,
    serperCallCount: Number.isFinite(fields.serperCallCount) ? fields.serperCallCount : null,
    estimatedCostUsd: Number.isFinite(fields.estimatedCostUsd) ? fields.estimatedCostUsd : null,
    ...
    // Shared with lib/serial-refinement/telemetry.js through one projection so
    // the two allowlists cannot drift. See
    // lib/model-evidence-store/telemetry-fields.js for why that matters.
