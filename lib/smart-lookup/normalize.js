...
// Explicit model, serial, and service-tag labels are the only zero-risk way
// to assign token roles, so they are honored before shape-based reasoning.
...
const IDENTIFIER_LABEL_PATTERN = /\b(serial\s*(?:number|no\.?|#)|serial|model\s*(?:number|no\.?|#)|model|service\s*tag|asset\s*tag|s\/n|sn)\s*[:#-]?\s*([A-Za-z0-9]+(?:[-./][A-Za-z0-9]+)*)/gi;

export function extractLabeledIdentifiers(query) {
  const source = String(query || '');
  let labeledModel = '';
  let labeledSerial = '';
  let labeledServiceTag = '';
  let residual = source;
  let providerQuery = source;
  ...
  for (const match of matches) {
    ...
    const isServiceTag = label === 'servicetag' || label === 'assettag';
    if (isSerial && !labeledSerial) labeledSerial = value;
    else if (isServiceTag && !labeledServiceTag) labeledServiceTag = value;
    else if (!isSerial && !isServiceTag && !labeledModel) labeledModel = value;
    residual = residual.replace(match[0], ' ');
    if (isSerial || isServiceTag) providerQuery = providerQuery.replace(match[0], ' ');
  }
  return {
    labeledModel,
    labeledSerial,
    ...
    providerQuery: normalizeWhitespace(providerQuery),
  };
}
...
export function classifySmartLookupQuery(value) {
  ...
  const enteredBrandMatch = query.match(
    /\bbrand\s*[:=]\s*([A-Za-z][A-Za-z0-9 &.'-]*?)(?=\s*(?:\||,|;|\b(?:serial|model|category|type)\b\s*[:=])|$)/i
  );
  const enteredBrand = enteredBrandMatch?.[1]?.trim() || '';
  ...
  let modelInfo = labeled.labeledModel
    ...
    : chooseModel(labeled.residual, alias);
  ...
  // Research-trigger policy (general-search-first).
  //
  // Formerly `researchEligible` was gated on `hasProductSignal`, a positive-
  // signal ALLOWLIST requiring a brand+category pair, a recognized brand/
  // category plus one descriptive token, or two unrecognized descriptive
  // tokens. That made local classification a gatekeeper on whether a query
  // deserved an answer at all -- exactly backwards, since the queries local
  // classification understands least (an unrecognized product name, a brand
  // with nothing else, a bare category) are the ones that most need live
  // research, not the ones that should be refused it. "iPhone 14" has a
  // real product name plus a real generation number and was refused research
  // outright because "14" does not count as a descriptive token and "iphone"
  // alone did not clear the two-token bar.
  //
  // The policy is now a narrow EXCLUSION list: a nonempty query is
  // research-eligible by default. Only two things withhold it --
  //   (1) genuinely unusable input (empty, keyboard-mash, pure noise --
  //       `isUnusable` above, deliberately conservative and unchanged), and
  //   (2) a query that should be handed off to the deterministic serial
  //       decoder / manufacturer lookup instead of researched
  //       (`isServiceTagHandoff` below) -- an explicit service/asset-tag
  //       identifies one exact unit through the manufacturer's own tool, not
  //       a model, so researching it risks a fabricated decode. This only
  //       withholds research when the tag is the ONLY signal in the query;
  //       a recognized model line, family, exact model, or brand+category
  //       alongside it still reaches research undiminished (e.g. "Dell
  //       OptiPlex 9020 service tag ABC1234" keeps its OptiPlex recognition).
  // Local classification (brand/category/family/model) remains fully
  // computed and returned -- it now serves as a speed/confidence optimization
  // (the deterministic fast path and provider hints below) and never again as
  // a precondition for whether research is allowed to run.
  const descriptiveTokens = collectDescriptiveTokens(labeled.providerQuery, alias);
  // Informational only as of the general-search-first policy above: no
  // longer gates `researchEligible`, kept for diagnostics/telemetry on how
  // much local signal a query carried even when research proceeded anyway.
  ...
  // A recognized model token (even partial) is a real product identity, not
  // a bare service tag -- "H4080BM" alone must keep reaching research even
  // though it happens to be 7 characters, the same shape looksLikeBareServiceTag
  // checks for. Only a token with NO other recognized identity at all is
  // handed off instead of researched.
  const hasNamedProductSignal = Boolean(
    exactModel || modelLineId || productFamily || (classifiedBrand && genericCategory)
    || modelInfo.modelCompleteness !== 'none'
  );
  // A labeled serial ("serial number 12345678", "Trane serial 1234567") is
  // the same handoff case as a service tag: it identifies one unit through
  // deterministic decoding, not a model, so it must not reach research when
  // it is the ONLY signal in the query. `labeled.labeledSerial` is already
  // available here (computed above by extractLabeledIdentifiers) -- the
  // `serialIdentity` field derived from it is assigned later in this
  // function, so it is referenced directly rather than waiting for that.
  const isServiceTagHandoff = Boolean(
    (serviceTagIntent || looksLikeBareServiceTag(query) || labeled.labeledSerial)
    && !hasNamedProductSignal
  );
  const researchEligible = !isUnusable && !isServiceTagHandoff;
  ...
  const residualIdentifiers = collectResidualIdentifiers(labeled.residual, alias, modelInfo.modelIdentity);
  const serialIdentity = labeled.labeledSerial ? compactModel(labeled.labeledSerial) : '';
  const serialToken = labeled.labeledSerial ? labeled.labeledSerial.toUpperCase() : '';
  const serviceTagIdentity = labeled.labeledServiceTag ? compactModel(labeled.labeledServiceTag) : '';
  ...
  return {
    ...
    serviceTagIdentity,
    providerQuery: labeled.providerQuery,
    ambiguousIdentifiers,
    specificityLevel,
    // General-search-first: providerEligible mirrors researchEligible above --
    // a nonempty query reaches the provider by default. Only genuinely
    // unusable input (empty, keyboard-mash, pure noise) or a bare
    // service/asset-tag with no other product signal (isServiceTagHandoff,
    // which belongs on the deterministic decoder/manufacturer lookup, not
    // research) withholds it. A bare recognized brand ("LG") or bare
    // recognized category ("refrigerator") is now eligible too -- local
    // classification is a speed/confidence hint for the deterministic fast
    // path, never a precondition for whether research is allowed to run.
