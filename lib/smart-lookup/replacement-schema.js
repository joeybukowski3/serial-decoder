import { compactModel, isSameModel, normalizeWhitespace } from './normalize.js';
...
function normalizeOption(value, labels) {
  ...
  return {
    ...
    priceRange: text(value.priceRange, 100) || 'N/A',
    retailerName: text(value.retailerName, 120) || 'N/A',
...
export function normalizeReplacementResult(raw, options = {}) {
  ...
  const itemSummary = {
    ...
    model: queryInfo.modelCompleteness === 'exact'
      ...
      : (returnedModel || null),
    category: text(summaryInput.category, 120) || queryInfo.genericCategory || null,
    ...
    originalPriceDisplay: text(summaryInput.originalPriceDisplay, 120) || 'N/A',
  };
  ...
  const replacementOptions = Array.isArray(raw.replacementOptions)
    ? raw.replacementOptions.map((item) => normalizeOption(item, labels)).filter(Boolean).slice(0, 3)
    : [];
