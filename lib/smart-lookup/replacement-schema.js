...
// `options.errorCode`/`options.providerAttempted` let a substituted deterministic
// card still report which failure it replaced. Without them a timeout-substituted
// card looked indistinguishable from a query that never attempted research at
// all, which both hid the failure from the response payload and left the browser
// unable to tell a retryable condition from a terminal one. Defaults preserve the
// original call shape for existing callers.
export function normalizeDeterministicReplacementResult(raw, queryInfo, timings, options = {}) {
  return normalizeReplacementResult(raw, {
    ...
    providerAttempted: options.providerAttempted ?? false,
    fallbackUsed: false,
