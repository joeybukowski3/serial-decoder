...
export function isManufacturerDomain(domain) {
  ...
  return Array.from(MANUFACTURER_DOMAINS).some(
    (manufacturerDomain) => normalized === manufacturerDomain
      || normalized.endsWith(`.${manufacturerDomain}`),
  );
}
...
export function buildModelQuery({ brand, model, category }) {
  const categoryTerm = category ? category : (brand ? 'appliance' : 'product');
  return [brand ? `"${brand}"` : '', `"${model}"`, categoryTerm].filter(Boolean).join(' ');
}
...
export async function searchModelWithSerper(input, options = {}) {
  ...
  if ((!brand && input?.allowBrandless !== true) || !model) {
