...
import { extractDomain, isManufacturerDomain } from '../serper/model-search.js';
...
function sourceMatchesManufacturer(source) {
  return isManufacturerDomain(extractDomain(source?.url));
}
...
function hasStrictManufacturerProductionStart(raw, sources) {
  ...
  return STRICT_PRODUCTION_LANGUAGE.some((pattern) => pattern.test(timingDescription))
    && sources.some((source) => sourceMatchesManufacturer(source));
}
