...
import {
  getSharedEvidenceStore,
  isStoreConsulted,
} from '../model-evidence-store/index.js';
import {
  beginStoreShadowRead,
  observeStoreShadow,
} from '../model-evidence-store/shadow.js';
...
function logEvidence(logger, result, requestContext = {}, persistentStore = null) {
  if (!logger?.info) return;
  try {
    logger.info(JSON.stringify({
      event: 'shared_model_evidence',
      // Phase 3B: compact persistent-store shadow summary, or null when no
      // shadow read ran. Deliberately passed as an argument rather than
      // attached to `result`, so the shared evidence object that is cached in
      // Redis and handed to the Smart Lookup / Serial Refinement adapters
      // stays byte-identical to Phase 2.
      persistentStore,
...
export async function lookupModelEvidence(input = {}, options = {}) {
  ...
  // ---------------------------------------------------------------------
  // Phase 3B: persistent evidence store, SHADOW ONLY.
  //
  // Placement is deliberate and matches the Phase 3A design (§12):
  //   local deterministic evidence -> Redis -> persistent store -> providers
  //
  // Consequences, both intentional and covered by tests:
  //   * A Redis hit returns above, so no shadow read happens on a cache hit.
  ...
  //   * A local verified record returns through the normal path; the store is
  //     a tier BELOW local deterministic evidence, permanently.
  //   * `requestContext.localOnly` returns above, so the 2s local-only
  //     diagnostics budget is untouched.
  //
  // The read is started here, before the multi-second provider work, and
  // awaited afterwards, so it adds no measurable latency while remaining
  // bounded by MODEL_EVIDENCE_DB_MAX_MS in the worst case.
  const storeEnv = options.env || process.env;
  let storeShadowTask = null;
  if (options.evidenceStore || isStoreConsulted(storeEnv)) {
    try {
      const evidenceStore = options.evidenceStore
        || await getSharedEvidenceStore(storeEnv);
      storeShadowTask = beginStoreShadowRead(
        { brand, model, category: searchCategory || category, modelIdentity },
        {
          store: evidenceStore,
          env: storeEnv,
          deadline,
          now: options.now,
          force: Boolean(options.evidenceStore),
        },
      );
    } catch (_) {
      // Constructing the store must never affect the lookup.
      storeShadowTask = null;
    }
  }

  const observeShadow = (liveResult) => observeStoreShadow(storeShadowTask, {
    sharedEvidence: liveResult,
    consumer: requestContext.consumer || purpose,
  ...
  try {
    ...
    // Observed AFTER the Redis write so the shadow summary can never be
    // persisted into the cached payload, and AFTER `result` is final so the
    // comparison sees exactly what the user-facing path produced.
    logEvidence(options.logger, result, requestContext, await observeShadow(result));
    return result;
  } catch (error) {
    ...
    // A provider failure is exactly when store coverage matters most, so the
    // shadow observation is emitted on this path too.
    logEvidence(options.logger, result, requestContext, await observeShadow(result));
    return result;
  }
}
