...
function aggregateIdentity(facts, requestedModel, local) {
  const precedence = ['exact', 'variant', 'family', 'mismatch', 'unknown'];
  ...
  return {
    model: localModel || (matchType === 'exact' ? requestedModel : null),
    normalizedModel: localModel
      ? compactModelToken(localModel)
      : (matchType === 'exact' ? compactModelToken(requestedModel) : null),
    matchType,
    deterministicExact: matchType === 'exact',
    matchedBy: local?.normalization?.usedValidatedAlternative ? 'exact-alias' : (localModel ? 'canonical-model' : null),
  };
}

function uniqueLifecycleYear(facts, eventType) {
...
function uniqueProductionEndYear(facts) {
  const years = [...new Set(facts
    .filter((item) =>
      item.identity.effectiveMatchType === 'exact'
...
function statusFor({ facts, extraction, evidence, conflict }) {
  const exact = facts.some((item) => item.identity.effectiveMatchType === 'exact');
...
export async function lookupModelEvidence(input = {}, options = {}) {
  ...
  const emptyBase = {
    ...
    requestedIdentity: {
      ...
      normalizedBrand,
      normalizedModel,
      searchedModels: normalizedModel ? [model] : [],
    },
    matchedIdentity: {
      model: null,
  ...
  try {
    local = await (options.localLookup || findLocalRefinementEvidence)({ brand, model });
  } catch (_) {
    local = null;
  }
  ...
  if (requestContext.localOnly) {
    const matchedIdentity = aggregateIdentity(localEvidenceFacts, model, local);
  ...
  try {
    ...
    extraction = await runEvidenceExtraction(
      { brand, model, category },
      evidence,
      { ...options, cache, deadline, signal: options.signal },
    );

    const facts = [...localEvidenceFacts, ...webFacts(extraction)];
    const matchedIdentity = aggregateIdentity(facts, model, local);
    const productionStart = uniqueLifecycleYear(facts, 'production_start');
    const productionEnd = uniqueProductionEndYear(facts);
    const discontinuation = uniqueLifecycleYear(facts, 'discontinuation');
    const conflict = productionStart.conflict || productionEnd.conflict || discontinuation.conflict;
    const classified = statusFor({ facts, extraction, evidence, conflict });
    const searchResults = [evidence.baseline, evidence.documentFocused].filter(Boolean);
    const searchCount = searchResults.filter((item) =>
    ...
    const result = {
      ...emptyBase,
      matchedIdentity: aggregateIdentity(localEvidenceFacts, model, local),
      facts: localEvidenceFacts,
