...
const CODE_TO_CATEGORY = Object.freeze({
  ...
  // Persistent model evidence store (Phase 3B). Deliberately mapped onto the
  // EXISTING cache_read_failure category rather than given a new one: the
  // store is architecturally a cache tier between Redis and the providers, and
  // a store failure must never appear on operational dashboards as a new class
  // of outage. No FAILURE_CATEGORIES entry is added.
  STORE_TIMEOUT: 'cache_read_failure',
  STORE_UNAVAILABLE: 'cache_read_failure',
  STORE_QUERY_ERROR: 'cache_read_failure',
  STORE_MALFORMED_ROW: 'cache_read_failure',
  STORE_NOT_CONFIGURED: 'cache_read_failure',
  STORE_NO_BUDGET: 'cache_read_failure',
  // An ambiguous stored identity is the alias-poisoning fail-safe firing. It
  // maps to the existing canonical_ambiguity category so it is counted with
  // every other identity ambiguity rather than hidden in a store-only bucket.
  STORE_AMBIGUOUS_IDENTITY: 'canonical_ambiguity',
});
