...
test.describe('Smart Lookup controller', () => {
  ...
  test('age renders with semantic date labels and does not request optional replacement lookup', async ({ page }) => {
    let lkqCalls = 0;
    await page.route('**/api/lkq-lookup', async (route) => {
      lkqCalls += 1;
    ...
    await expect(page.locator('#panel-smart input[type="checkbox"]')).toHaveCount(0);
    await expect(page.locator('#smart-lookup-replacement-panel')).toHaveCount(0);
    expect(lkqCalls).toBe(0);
  });
  ...
  test('Smart Lookup forms stay within their containers on desktop and mobile', async ({ page }) => {
    for (const viewport of [{ width: 1280, height: 800 }, { width: 375, height: 812 }]) {
      await page.setViewportSize(viewport);

      for (const target of [
        { url: 'http://localhost:3001/index.html?mode=smart#panel-smart', container: '#panel-smart', submit: '[data-smart-lookup-submit="1"]' },
        { url: 'http://localhost:3001/smart-lookup.html', container: '.lookup-box', submit: '#smartLookupBtn' },
      ]) {
        await page.goto(target.url);
        const measurements = await page.evaluate(({ container, submit }) => {
          const containerBox = document.querySelector(container).getBoundingClientRect();
          const inputBox = document.querySelector('#smart-lookup-input').getBoundingClientRect();
          const submitBox = document.querySelector(submit).getBoundingClientRect();
          return {
            viewportOverflow: document.documentElement.scrollWidth > window.innerWidth,
            inputContained: inputBox.left >= containerBox.left - 1 && inputBox.right <= containerBox.right + 1,
            submitContained: submitBox.left >= containerBox.left - 1 && submitBox.right <= containerBox.right + 1,
          };
        }, target);
